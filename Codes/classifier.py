...
parser = argparse.ArgumentParser(description='Running random forest classifer to classify PD subjects from health controls.')

parser.add_argument('--experiment_No', help='Experiment number according to the paper. Experiment 1, 2 and 3 can be run using this script. Experiment 5 (validation) will run automatically along with experiment 1 and 2', type=int,choices=[1, 2, 3], default=1)
...
def classify(df):
	...
	#if UPDRS == True:
	get_prc(train_x,train_y,subject_list)
...
def get_cv_results(train_x,train_y,subject_list,folds=5):
	...
	np.savetxt(out_path+'/confusion_matrix_'+title+'.csv', conf_matrix, fmt='%s', delimiter=',')
	...
	plt.title('ROC of '+title+' vs CT')
	plt.savefig(out_path+'mean_roc_curve_'+title+'_vs_CT.pdf', bbox_inches='tight')
	plt.show()

	accuracies = [['F1', mean_test_acu, std_test_acu / 2], ['auc', mean_auc, std_auc / 2 ]]

	np.savetxt(out_path+'/accuracies_'+title+'_vs_ct_all_PPMI.csv', accuracies, fmt='%s', delimiter=',')"""

	return models,base_fpr, mean_tprs,mean_auc, std_auc, mean_test_acu, std_test_acu

def get_roc_for_selected_fetres(df,title):

	accuracy_array = []
	plt.figure(figsize=(5, 5))
	
	header_list = list(df)
	sus_header = [x for x in header_list if any(kw in x for kw in ['meansusc'])]
	SASD_header = [x for x in header_list if any(kw in x for kw in ['8','9'])]
	...
	train_x = train_x_original[sus_header]

	models,base_fpr, mean_tprs,mean_auc, std_auc, mean_test_acu, std_test_acu = get_cv_results(train_x,train_y,subject_list)
	plt.plot(base_fpr, mean_tprs, 'r',label='QSM (AUC = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc), lw=2)

	accuracy_array.append(["QSM", mean_auc,std_auc, mean_test_acu, std_test_acu])
	...
	plt.axes().set_aspect('equal', 'datalim')
	plt.legend(loc="lower right")
	plt.title('ROC of '+title+' vs CT for selected features')
	plt.savefig(out_path+'selected_features_mean_roc_curve_'+title+'.pdf', bbox_inches='tight')

	plt.show()

def random_forest_validation_once(models):

	
	df=pd.read_csv('../Data/combat/all/combined_BL/Combined_with_earlyPD.csv')

	#df=pd.read_csv('../Data/combat/all/combined_BL/VTASN_traindata.csv')

	valid_df=pd.read_csv('../Data/combat/all/combined_BL/VTASN_testdata_all.csv')

	train_x,train_y,subject_list = just_split(df)

	#new_header = get_new_header([8,9])
	#train_x = train_x[new_header]
	...
	T_P = confusion_matrix(test_y, predictions)[0,0]
	F_P = confusion_matrix(test_y, predictions)[0,1]
	F_N = confusion_matrix(test_y, predictions)[1,0]
	T_N = confusion_matrix(test_y, predictions)[1,1]

	conf_matrix = [T_P,F_P,F_N,T_N]
	np.savetxt(out_path+'/confusion_matrix_VTASN.csv', conf_matrix, fmt='%s', delimiter=',')

	f1_value = f1_score(test_y, np.around(predictions),average='micro')
	...
	plt.title('Validation ROC curve - '+title+' vs CT')
...
def random_forest_validation(models):
	...
	for model in models:
		...
		#fpr, tpr, _ = plot_precision_recall_curve(test_y, y_score)
		roc_value = roc_auc_score(test_y, y_score, average = "micro")
		f1_value = f1_score(test_y, np.around(y_score),average='micro')
		f1.append(f1_value)
		auc.append(roc_value)
	...
	# Train and Test Accuracy
		...
		T_P = T_P + confusion_matrix(test_y, predictions)[0,0]
		F_P = F_P + confusion_matrix(test_y, predictions)[0,1]
		F_N = F_N + confusion_matrix(test_y, predictions)[1,0]
		T_N = T_N + confusion_matrix(test_y, predictions)[1,1]
	...
	plt.legend(loc="lower right")
	plt.title('Validation ROC curve - '+title+' vs CT')
	plt.savefig(out_path+'mean_roc_curve_VTASN.pdf', bbox_inches='tight')
	plt.show()

	accuracies = [['F1', mean_test_acu, std_test_acu / 2], ['auc', mean_auc, std_auc / 2 ]]

	np.savetxt(out_path+'/accuracies_PD_vs_ct_all_vtasn.csv', accuracies, fmt='%s', delimiter=',')

def get_prc(train_x,train_y,subject_list,folds=5):
	#This function takes a training data set and a classifier and run cross validation.
	#Within each split, it calculates PRC curves
	#train_x: panda df with features
	#train_y: panda df with classes
	#clf: a classifier 
	#subject_list: panda df with subject names
...
if exp_no == 1:
	...
	title = 'PD'
	UPDRS = False
	df=pd.read_csv('../Data/QSM_Data_for_Dimuthu.csv')
	models = classify(df)
...
if exp_no == 3:
	...
	UPDRS = False
	df=pd.read_csv('../Data/QSM_Data_for_Dimuthu.csv')
	get_roc_for_selected_fetres(df,title)
