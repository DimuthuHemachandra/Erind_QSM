...
def get_demo_info():
  age = pd.read_csv("../Data/VTASN/age.csv")

  print(age)

  val = stats.ttest_ind(age["CT"], age["PD"],nan_policy='omit')

  print(val)

  print("CT mean age:",age['CT'].mean(skipna=True))
  print("std:",age['CT'].std(skipna=True))

  print("PD mean age:",age['PD'].mean(skipna=True))
  print("std:",age['PD'].std(skipna=True))
...
def get_combined_table(VTASN_csv_file_list,PPMI_csv_file_list): 
  ...
  for i in range(len(VTASN_csv_file_list)):
    ...
    df_combined = pd.concat([PPMI_df, VTASN_new_df],sort=False)


    print("These files are being added together and processing with combat:")
    print(file[i])

    df_combined.to_csv("../Data/combat/all/PPMI_with_VTASN/"+file[i]+".csv", sep=',', index=False)
...
def strip_PPMI():
  ...
  file_list = glob.glob("../Data/combat/all/combat_PPMI_with_VTASN/"+'*168*'+'.csv')
  file_list.sort()
  ...
  for i in range(len(file_list)):
    ...
    df_vtsn.to_csv("../Data/combat/all/testdata_VTASN/"+file[i]+".csv", sep=',', index=False)
    ...
    df_ppmi_fixed.to_csv("../Data/combat/all/train_PPMI_with_VTASN/"+file[i]+".csv", sep=',', index=False)
    #print(PD)
